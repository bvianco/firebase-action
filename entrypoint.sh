...
if [ -n "$GCP_SA_KEY" ]; then
  if echo "$GCP_SA_KEY" | jq empty 2>/dev/null; then
    echo "Storing GCP_SA_KEY in $HOME/gcp_key.json"
    echo "$GCP_SA_KEY" > "$HOME/gcp_key.json"
  else
    echo "Storing the decoded GCP_SA_KEY in $HOME/gcp_key.json"
    echo "$GCP_SA_KEY" | base64 -d > "$HOME/gcp_key.json" # If encoded base64 key, decode and save
  fi
...
bash -c "source $HOME/.nvm/nvm.sh && firebase $*"
